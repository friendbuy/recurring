...
class Coupon extends RecurlyData {
  ...
  create(options, callback) {
    ...
    if (
      options.coupon_type &&
      Coupon.validCouponTypes.indexOf(options.coupon_type) === -1
    ) {
      throw new Error(
        `coupon_type must be one of ${JSON.stringify(Coupon.validCouponTypes)}`
      );
    }
    if (
      options.discount_type &&
      Coupon.validDiscountTypes.indexOf(options.discount_type) === -1
    ) {
      throw new Error(
        `discount_type must be one of ${JSON.stringify(
          Coupon.validDiscountTypes
        )}`
      );
    }
    if (
      options.duration &&
      Coupon.validDurations.indexOf(options.duration) === -1
    ) {
      throw new Error(
        `duration must be one of ${JSON.stringify(Coupon.validDurations)}`
      );
    }
    if (
      options.free_trial_unit &&
      Coupon.validFreeTrialUnits.indexOf(options.free_trial_unit) === -1
    ) {
      throw new Error(
        `free_trial_unit must be one of ${JSON.stringify(
          Coupon.validFreeTrialUnits
        )}`
      );
    }
    if (
      options.temporal_unit &&
      Coupon.validTemporalUnits.indexOf(options.temporal_unit) === -1
    ) {
      throw new Error(
        `temporal_unit must be one of ${JSON.stringify(
          Coupon.validTemporalUnits
        )}`
      );
    }
    if (
      options.redemption_resource &&
      Coupon.validRedemptionResources.indexOf(options.redemption_resource) ===
        -1
    ) {
      throw new Error(
        `redemption_resource must be one of ${JSON.stringify(
          Coupon.validRedemptionResources
        )}`
      );
    }

    if (
      Object.prototype.hasOwnProperty.call(options, "applies_to_all_plans") &&
      !options.applies_to_all_plans
    ) {
      // Defaults to true. If it's false, plan_codes must be an array.
      if (!options.plan_codes && !Array.isArray(options.plan_codes)) {
        throw new Error(
          "coupons that do not apply to all plans must specify plan_codes"
        );
      }
    }
  ...
  redeem(options, callback) {
    ...
    if (!options.account_code) {
      throw new Error(
        'coupon redemption options must include "account_code" parameter'
      );
    }
    if (!options.currency) {
      throw new Error(
        'coupon redemption options must include "currency" parameter'
      );
    }
