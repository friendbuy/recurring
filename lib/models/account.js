...
class Account extends RecurlyData {
  constructor(recurring) {
    super({
      ...
      properties: [
        ...
        "account_code",
        "address",
        "bill_to", // self or parent (if parent requires parent_account_code)
        "cc_emails",
        "closed_at",
        "company_name",
        "created_at",
  ...
  create(data, callback) {
    ...
    this.post(Account.ENDPOINT, body, (err, response, payload) => {
      ...
      if (response.statusCode === 422) {
        ...
        if (
          _.get(payload, "error.symbol") === "taken" &&
          _.get(payload, "error.field") === "account.account_code"
        ) {
  ...
  update(callback) {
    const newData = {
      ...
      parent_account_code: this.parent_account_code,
    };
  ...
  createInvoice(callback) {
    ...
    this.post(uri, null, (err, response, payload) => {
      ...
      callback(null, payload.charge_invoice);
    });
  }
  ...
  fetchBalance(callback) {
    const uri = this._resources.balance || `${this.href}/balance`;

    this.get(uri, null, (err, response, payload) => {
