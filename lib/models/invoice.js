...
class Invoice extends RecurlyData {
  ...
  create(options, callback) {
    if (!options.account_code) {
      throw new Error('invoice options must include "account_code" parameter');
    }

    if (!options.currency) {
      throw new Error('invoice options must include "currency" parameter');
    }

    const body = data2xml(Plan.SINGULAR, options);
    this.post(Invoice.ENDPOINT, body, (err, response, payload) => {
  ...
  markFailed(callback) {
    ...
    if (!uri && !this.invoice_number) {
      throw new Error(
        "cannot mark invoice as failed without an invoice_number"
      );
    }
    ...
    this.put(uri, null, (err, response, payload) => {
      ...
      this.inflate(payload.charge_invoice);
      callback(null, this);
    });
  }
}
