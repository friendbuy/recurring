...
class RecurlyData {
  ...
  all(filter, callback) {
    ...
    if (!this.enumerable) {
      return callback(
        new Error(`${this.constructor.name} does not support .all()`)
      );
    }
  ...
  fetchAll(modelName, uri, filter, callback) {
    ...
    var result = [];
    iterators.forEachAsync(
      iterator,
      function(err, item, next) {
        if (err) {
          return callback(err);
        }
        result.push(item);
  ...
  execute(options, callback) {
    ...
    this._recurring.request(options, function(err, response, body) {
      ...
      if (response.statusCode === 401) {
        return callback(
          new Error("Your API key is missing or invalid"),
          response,
          {}
        );
      }
  ...
  iterator(filter, endpoint) {
    ...
    let uri = filter
      ? `${endpoint}?${querystring.stringify(filter)}`
      : endpoint;
