...
describe("Plan", () => {
  ...
  it("can create a plan addon", done => {
    ...
    plan.createAddon(data, (err, newAddon) => {
      ...
      newAddon.name.must.equal(data.name);
      done();
    });
  });
  ...
  describe("refunds", () => {
    ...
    it("can issue an open amount refund for a specific amount against an invoice", function(done) {
      ...
      invoice.refund(refundOptions, err => {
        ...
        invoice.subtotal_in_cents.must.equal(
          refundOptions.amount_in_cents * -1
        );
        done();
      });
    });
    ...
    it("can issue an open amount refund for the full amount against an invoice", function(done) {
      ...
      invoice.refund(err => {
        ...
        invoice.total_in_cents.must.equal(
          refundableInvoice.total_in_cents * -1
        );
        done();
      });
    });
